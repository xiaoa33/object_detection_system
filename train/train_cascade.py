...
def parse_args():
    ...
    parser.add_argument("--max_pos", type=int, default=200, help="训练集正样本数")
    parser.add_argument("--max_neg", type=int, default=400, help="训练集初始负样本数")
    parser.add_argument("--target_fpr", type=float, default=0.1, help="目标整体假正率")
...
# 在计算特征时，强制转换为 float32 节省一半内存
    if os.path.exists(args.features_cache) and not args.force_recompute:
        ...
        imgs_pos_val, imgs_neg_val = load_val_data(max_pos=50, max_neg=100)
